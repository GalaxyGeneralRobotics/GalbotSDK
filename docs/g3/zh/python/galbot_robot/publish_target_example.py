"""
G3 PublishTarget menu example for SDK mirror SingoriXTarget.
...
def main():
    ...
    head_joint_names = robot.get_joint_names(True, [G3JointGroup.head])
    left_arm_joint_names = robot.get_joint_names(True, [G3JointGroup.left_arm])
    ...
    while True:
        ...
        if command == "joint":
            target = build_joint_target(G3JointGroup.head, head_single_joint, [0.2], joint_time_s)
        ...
        if command == "mixed_pose":
            ...
            target = merge_targets(
                [
                    build_joint_target(G3JointGroup.head, head_single_joint, [0.2], joint_time_s),
                    build_joint_target(G3JointGroup.left_arm, arm_single_joint, [0.1], joint_time_s),
